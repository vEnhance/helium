...
class ResultPrinter:
	...
	def get_table(self, heading = None, num_show = None,
			...
			float_string = "%4.2f", int_string = "%4d", total_string = "%7.2f"):
...
def HMMT_text_report(queryset = None,
	...
	output += RP(rows).get_table(heading = "Overall Individual Awards",
			...
			float_string = "%5.2f", int_string = "%5d", total_string = "%8.3f")
	output += "\n"
	...
	for exam in team_exams:
		...
		output += RP(rows).get_table(heading = unicode(exam),
				num_show = num_show, num_named = num_named, zero_pad = zero_pad,
				float_string = "%2.0f", int_string = "%2d", total_string = "%5d")
		output += "\n"
	...
	output += RP(rows).get_table("Mathlete Aggregate",
			num_show = num_show, num_named = num_named, zero_pad = zero_pad,
			float_string = "%6.2f", int_string = "%6d", total_string = "%8.3f")
	output += "\n"
	...
	output += RP(rows).get_table("Sweepstakes",
			...
			float_string = "%7.2f", int_string = "%7d", total_string = "%8.3f")
	output += "\n"
